...
Is the effect of increasing temperature more pronounced in some months? W e will use `facet_wrap()` to produce a seperate scatter plot for each month, again with a smoothing line. 
...
Let us construct a confidence interval for the average annual delta since 2011, both using a formula and using a bootstrap simulation with the `infer` package; we are only interested in what is happening  between 2011-present.
...
Let us calculate the average net approval rate (approve- disapprove) for each week since he got into office. I want you plot the net approval, along with its 95% confidence interval. There are various dates given for each poll, please use `enddate`, i.e., the date the poll ended.
...
Lets us compare the confidence intervals for `week 3` and `week 25`:
...
Let us reproduce the following two graphs:
